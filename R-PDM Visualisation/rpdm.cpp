...
int main(int argc, char* argv[])
{
    ...
    for (string line; getline(par_input, line); ) {
        ...
        string word1, word2;
        //getline(input_line, word1, ' ');
        //getline(input_line, word2, ' ');
        input_line >> word1 >> word2;
        if (word1 == "Nx") { Par.Nx = stoi(word2); ++check; }
    ...
    //Par.Nt = ceil(2 * (Par.D_MV + Par.D_OV) * Par.T * pow(Nx / Par.L, 2));
    //Par.dt = Par.T / Par.Nt;
    ...
    solve(x, t, C_MV_0, C_OV_0, t_data, Data_C_MV, Data_C_OV, Data_phi, Data_E, Data_k2, Par);

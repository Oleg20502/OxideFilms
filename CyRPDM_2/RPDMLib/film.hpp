...
class Film
{
    ...
    public:
        ...
        int init(int mode = 0)
        {
            ...
            if (mode == 1){
                init_C(C_MV_0, C_OV_0, x, round(0.1 * L / h), round(0.1 * L / h));
            }
            return init(C_MV_0, C_OV_0);
        }
        ...
        static void init_C(vector<double>& C_MV_0, vector<double>& C_OV_0,
                           vector<double>& x, int s1, int s2)
        {
            int Nx = x.size();
            for (int i = 0; i < Nx; ++i) {
                if (i < s1) {
                    C_MV_0[i] = 1.50 / x[s1] * x[i];
                }
                else {
                    C_MV_0[i] = 1.50;
                }
            ...
            for (int i = 0; i < Nx; ++i) {
                if (i <= Nx - s2) {
                    C_OV_0[i] = (2.10 - (2.10 - 1.85) / x[Nx - s2] * x[i]);
                }
                else {
                    C_OV_0[i] = 1.85 * (x[Nx-1] - x[i]) / (x[Nx-1] - x[Nx-s2]);
                }
            }
        }
        ...
        vector<double> & get_t_data() { return t_data; }

        vector<double> & get_Data_k2() { return Data_k2; }

        vector<vector<double>> & get_Data_C_MV() { return Data_C_MV; }

        vector<vector<double>> & get_Data_C_OV() { return Data_C_OV; }

        vector<vector<double>> & get_Data_phi() { return Data_phi; }

        vector<vector<double>> & get_Data_E() { return Data_E; }
